...
Utility to send an e-mail from file or standard input to an SMTP server.

Usage:
smtpsend /SERVER=server [/PORT=portnumber] [/HELO=heloname] [/FROM=sender]
    [/FILE=filename [/DELETE]] [/TLS [/IFSUPPORTED] [/UNSAFE] | /SSL [/UNSAFE]]
    [/RCPT=receipent1 [/RCPT=receipent2] [...]]");
        ...
        try
        {
            ...
            for (; ; )
            {
                ...
                if (tls && smtp is not AuthenticatedStream)
                {
                ...
                else
                {
                    recipents.
                        AddRange(header
                            ...
                            .SelectMany(s => s.Substring("To: ".Length).Split(',').Select(address => address.TrimStart('<').TrimEnd('>'))));
                }
            }
